    ...
    while (1)
    {
		...
		if(writerInput.readyForMoreMediaData){
			...
			if (i >= [array count]) {
				...
				buffer = [HJImagesToVideo pixelBufferFromCGImage:[HJImagesToVideo nsImageToCGImageRef:array[i]] size:size];
			}
			...
			if (buffer) {
                ...
                if (shouldAnimateTransitions && i + 1 < array.count) {
                    ...
                    for (double j = 1; j < framesToFadeCount; j++) {
                        ...
                        buffer = [HJImagesToVideo crossFadeImage:[HJImagesToVideo nsImageToCGImageRef:array[i]]
                                                         toImage:[HJImagesToVideo nsImageToCGImageRef:array[i + 1]]
                                                          atSize:size
