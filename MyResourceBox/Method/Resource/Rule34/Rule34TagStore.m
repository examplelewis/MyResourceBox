...
@implementation Rule34TagStore

static Rule34TagStore *request;
+ (Rule34TagStore *)defaultManager {
    static dispatch_once_t predicate;
    
    dispatch_once(&predicate, ^{
        request = [[Rule34TagStore alloc] init];
    });
...
- (void)fetchAllTags {
    ...
    [[HttpManager sharedManager] getRule34TagsWithPid:pid success:^(NSArray *array) {
        if (array.count == 0) {
...
- (void)processNeededTags {
    ...
    dispatch_main_sync_safe(^() {
        MyAlert *alert = [[MyAlert alloc] initWithAlertStyle:NSAlertStyleCritical];
        [alert setMessage:@"整理Rule34的Tag 完成" infomation:nil];
        [alert setButtonTitle:@"好" keyEquivalent:@"\r"];
        [alert runModel];
    });
}
