...
- (void)analyzeFileAtPath:(NSString *)filePath {
    ...
    for (NSInteger i = 0; i < archive.subresources.count; i++) {
        ...
        //        if (![[MRBUserManager defaultUser] mimeTypeExistsInFormats:format] || resource.data.length < 25600) { // 忽略 不符合类型 或者 小于25KB 的文件
        //            continue;
        //        }
...
- (void)finishAnalyzing {
    ...
    for (NSInteger i = 0; i < targetFolderPaths.count; i++) {
        ...
        NSArray<NSString *> *targetFolderImageFilePaths = [[MRBFileManager defaultManager] getFilePathsInFolder:targetFolderPath specificExtensions:[MRBUserManager defaultUser].web_archive_mime_type];
