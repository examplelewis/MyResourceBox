...
- (instancetype)init {
    ...
    if (self) {
        ...
        NSString *filePath = [[MRBUserManager defaultUser].path_root_folder stringByAppendingPathComponent:@"FetchResource/Rule34LatestPost.plist"];
...
- (void)fetchFatePostsSucceed {
    // 更新 latestPost
    if (!!newestPost) {
        NSString *dest = [[MRBUserManager defaultUser].path_root_folder stringByAppendingPathComponent:@"FetchResource/Rule34LatestPost.plist"];
